...
async def check_buy_signal(exchange, df, symbol, warning_list):
    ...
    ma90_up_count = ma90_intensity_ok.tail(10).sum() if actual_up_count_90 >= 3 else 0
...
# ---------- [기존 유지 및 보강] 30분봉 기준 S+ 수급 ----------
    ...
    last_4_status = (df['high'] >= df['ma14']).tail(4)
    ...
    ###################  TYPE3 START ###################
    # 1. 대전제: 완전 역배열 및 과매도 이력 확인
    is_inverse_align = (ma185_val > ma90_val > ma40_val)
    was_oversold_start = (df['rsi'].iloc[-30:] <= 40).any()
    
    # 185선 장기 하락 대전제 (최근 200봉 중 60% 이상 하락세 유지)
    if is_inverse_align and was_oversold_start:
        desc_range = 200
            ...
            if is_high_pos_185:
                ...
                if is_slide_setup_t3 and is_candle_clean and has_recovered_target:
                    ...
                    if symbol in s_plus_tracker:
                        ...
                        # //======== [2026-04-30 수정: 인덱스 가드 추가 및 다음 봉부터 돌파 인정] ========//
                        if len(df) > tracker.get('index', 0):
                            if not tracker.get('triggered_s', False):
                                target_p = tracker.get('close', tracker.get('target', 0))
                                if curr_price > target_p:
                                    tracker['triggered_s'] = True
                                    tracker['index'] = len(df)
                                    set_time = tracker.get('set_time', '??:??:??')
                                    data_dict['grade'] = 'S'
                                    return True, f"⭐ [TYPE3-S] {set_time} 바닥탈환 돌파", "S", data_dict
                        # //==============================================================================//
                    else:
                        data_dict['grade'] = 'S+'
                        # //======== [2026-04-30 수정: S+ 기록 시 현재 인덱스와 실시간 가격 정확히 박제] ========//
    ...
    if gap_14_40_pct <= 0:
        ...
        is_t4_rebound = (-3.0 <= gap_14_40_pct <= 0) and (ma14_up_count >= 1) and is_bullish_breakout and is_valid_convergence
    ...
    if is_near_ma40 and is_upward_trend and disparity_40 <= 0.07 and data_dict.get('dynamic_rise_YN') != 'Y':
        ...
        if strict_descending and strict_stabilized and is_t1_structure_ready:
            ...
            if is_slope_strong or is_slope_dense:
                ...
                ###### [TYPE1 수정: 185선-90선 수렴 추세(최근 6봉)만 엄격하게 확인] ######
                hist_disp_90 = [abs(df['ma90'].iloc[-i] - df['ma185'].iloc[-i]) / df['ma185'].iloc[-i] if df['ma185'].iloc[-i] > 0 else 999 for i in range(1, 8)]
                
                # 90선이 185선으로 수렴하거나 유지되는 횟수 (최소 5회 이상 요구)
                conv_cnt_90 = sum(1 for i in range(6) if hist_disp_90[i] <= hist_disp_90[i+1])
                
                # //======== [타입 1: 40선(X) -> 90선 수렴(conv_cnt_90)만 진입 조건으로 사용] ========//
                if disparity_gold <= 0.015 and conv_cnt_90 >= 5 and is_converging_5_40 and (-1.0 <= gap_5_40_pct <= 1.0) and (ma90_up_count >= 4):
                    if is_slide_setup_t1 and is_candle_clean and has_recovered_target:
                        if symbol in s_plus_tracker:
                            tracker = s_plus_tracker[symbol]
                            # //======== [2026-04-30 수정: TYPE 1 인덱스 가드] ========//
                            if len(df) > tracker.get('index', 0):
                                target_p = tracker.get('close', 0)
                                if curr_price > target_p:
                                    data_dict['grade'] = 'S'
                                    tracker['index'] = len(df)
        ...
        try:
            ...
            if vol_30m < 1.2 and profit_rate_pct > 7.0:
                ...
                if c2_candle['close'] >= c2_candle['open'] and abs_peak_idx > 0:
                    ...
                    if c1_candle['close'] > c1_candle['open']:
                        ...
                        if curr_3m_p < c1_mid_p and curr_3m_data['close'] < curr_3m_data['open']:
                            return True, f"🚨 [긴급익절] 수익 경신 멈춤(C1허리이탈) | 수익: {profit_rate_pct:.2f}%", True
                ...
                if is_2_neg_3m:
                    ...
                    if is_2_neg_3m: 
                        return True, f"{reason_2_neg_3m} | 수익: {profit_rate_pct:.2f}%", True
                ...
                if profit_rate_pct >= 13.0 and curr_3m_p <= purchase_price * 1.13:
                    return True, f"🚨 [수익사수] 13% 마지노선 매도 | 수익: {profit_rate_pct:.2f}%", True
